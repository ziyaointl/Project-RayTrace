...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil and vals is not nil:
            local_frame.define(formals.first, vals.first)
            vals = vals.second
            formals = formals.second
        if formals is not nil or vals is not nil:
            raise SchemeError('Incorrect number of arguments supplied to lambda')
        return local_frame
        # END PROBLEM 11
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Wrong number of argument supplied to '
                              + 'built-in function {}'.format(self.name))
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # END PROBLEM 6
    # BEGIN PROBLEM 10
    return bind_procedure(target, expressions.second, env, LambdaProcedure)
    # END PROBLEM 10
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        curr_binding = bindings.first
        check_form(curr_binding, 2, 2)
        formals = Pair(curr_binding.first, formals)
        args = Pair(scheme_eval(curr_binding.second.first, env), args)
        bindings = bindings.second
    check_formals(formals)
    return env.make_child_frame(formals, args)
    # END PROBLEM 15

def bind_procedure(target, body, env, procedure_type):
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        formals = target.second
        name = target.first
        env.define(name, procedure_type(formals, body, env))
        return name
...
def do_define_macro(expressions, env):
    ...
    check_form(expressions, 2)
    target = expressions.first
    return bind_procedure(target, expressions.second, env, MacroProcedure)
    # END Problem 21
